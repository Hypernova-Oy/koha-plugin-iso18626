package Koha::Plugin::ISO18626::Install;
...
use Koha::Plugin::ISO18626::OpeningHours;
    ...
    eval {
        if ( $cgi && $cgi->param('save') ) {
            ...
            unless ($openinghours_loop_error) {
                $openinghours_loop_error = Koha::Plugin::ISO18626::OpeningHours::validate($openinghours);
            }
            ...
            $template->param(
                ...
                plugin_version => $Koha::Plugin::ISO18626::VERSION,
            );
...
sub bug33503_rewrite_openapi_file_refs {
    ...
    Koha::Plugin::ISO18626::URLLib::convert_refs_to_absolute($c, $absolute_base_dir_path.'/');
